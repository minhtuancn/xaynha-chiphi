...
import { Download, TrendingUp, Wallet, Package, Users, HardHat, FileText } from "lucide-react";
...
function FinancialReportTab({ data }: { data: FinancialReport | null }) {
  ...
  const exportPDF = useCallback(async () => {
    ...
    try {
      ...
      pdf.save("bao-cao-tai-chinh.pdf");
    } catch (e) {
      console.error("PDF export failed:", e);
    } finally {
      setExporting(false);
...
function WorkersReportTab({ data }: { data: WorkerReport | null }) {
  ...
  const exportPDF = useCallback(async () => {
    ...
    try {
      ...
      const pdfH = (imgProps.height * pdfW) / imgProps.width;
      pdf.addImage(imgData, "PNG", 0, 0, pdfW, pdfH);
      pdf.save("bao-cao-nhan-cong.pdf");
    } finally {
      setExporting(false);
    }
  }, []);
